    ...
    <div style={S.app}>
      ...
      <div style={{ ...S.wrap, paddingTop: 32, paddingBottom: 100 }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ ...S.tag, marginBottom: 6 }}>⚡ Provocări reale</div>
          <h2 style={S.h2}>Challenges săptămânale</h2>
          <p style={{ color: '#555', fontSize: 14, margin: '6px 0 0', lineHeight: 1.6 }}>
            Aplică ce ai învățat în viața reală. Bifează ce ai finalizat.
          </p>
        </div>

        {/* Challenge săptămâna asta */}
        <div style={{ background: 'linear-gradient(135deg,#00FF8715,#00C6FF10)', border: '1px solid #00FF8730', borderRadius: 20, padding: 20, marginBottom: 20 }}>
          <div style={{ ...S.tag, color: '#00FF87', marginBottom: 10 }}>⭐ Challenge-ul săptămânii</div>
          ...
          <Btn onClick={() => toggle(weekChallenge.id)}
            color={completed.includes(weekChallenge.id) ? '#555' : '#00FF87'}
            style={{ marginTop: 14, padding: '12px' }}>
            {completed.includes(weekChallenge.id) ? '✓ Finalizat!' : 'Marchează ca finalizat'}
          </Btn>
        </div>

        {/* Toate challengeurile */}
        <div style={{ ...S.tag, marginBottom: 12 }}>
          Toate challengeurile · {completed.length}/{CHALLENGES.length} completate
        </div>
          ...
          {CHALLENGES.map(c => {
            ...
            return (
              <div key={c.id} onClick={() => toggle(c.id)}
                ...
                <div style={{ width: 22, height: 22, borderRadius: '50%', border: `2px solid ${done ? '#00FF87' : 'rgba(255,255,255,.12)'}`, background: done ? '#00FF87' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
